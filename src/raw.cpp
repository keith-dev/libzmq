...
zmq::raw_t::raw_t (class ctx_t *parent_, uint32_t tid_, int sid_) :
...
void zmq::raw_t::xattach_pipe (pipe_t *pipe_,
...
void zmq::raw_t::xpipe_terminated (pipe_t *pipe_)
{
...
void zmq::raw_t::xread_activated (pipe_t *pipe_)
{
    _fq.activated (pipe_);
}

int zmq::raw_t::xsend (msg_t *msg_)
{
...
int zmq::raw_t::xsetsockopt (int option_,
                                const void *optval_,
                                size_t optvallen_)
{
    switch (option_) {
...
int zmq::raw_t::xrecv (msg_t *msg_)
{
    if (_prefetched) {
        if (!_routing_id_sent) {
...
bool zmq::raw_t::xhas_out ()
{
    //  In theory, RAW socket is always ready for writing. Whether actual
    //  attempt to write succeeds depends on which pipe the message is going
    //  to be routed to.
    return true;
}

void zmq::raw_t::identify_peer (pipe_t *pipe_, bool locally_initiated_)
{
