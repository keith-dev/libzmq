...
static void test_raw_to_dealer ()
{
    int rc;
    char my_endpoint[MAX_SOCKET_STRING];

    //  We'll be using this socket in raw mode
    void *stream = test_context_socket (ZMQ_RAW);
    ...
    int enabled = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (stream, ZMQ_RAW_NOTIFY, &enabled, sizeof (enabled)));
    ...
    //  Test large messages over RAW socket
#define size 64000
    uint8_t msgout[size];
...
static void test_raw_to_stream ()
{
    char my_endpoint[MAX_SOCKET_STRING];
    //  Set-up our context and sockets

    void *server = test_context_socket (ZMQ_RAW);
    int enabled = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server, ZMQ_RAW_NOTIFY, &enabled, sizeof (enabled)));
    bind_loopback_ipv4 (server, my_endpoint, sizeof my_endpoint);

    void *client = test_context_socket (ZMQ_RAW);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, ZMQ_RAW_NOTIFY, &enabled, sizeof (enabled)));
    ...
    // FIXME: why does this block? Bug in RAW disconnect notification?
...
int main ()
{
    ...
    UNITY_BEGIN ();
    RUN_TEST (test_raw_to_dealer);
    RUN_TEST (test_raw_to_stream);
    return UNITY_END ();
}
